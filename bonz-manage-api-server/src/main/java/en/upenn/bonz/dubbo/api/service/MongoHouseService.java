...
import en.upenn.bonz.dubbo.api.pojo.MongoHouse;
import en.upenn.bonz.dubbo.api.vo.map.MapHouseDataResult;
import en.upenn.bonz.dubbo.api.vo.map.MapHouseXY;
...
public class MongoHouseService {
    ...
    static {
        BAIDU_ZOOM.put(19, 20d / 1000); // unit: km
    ...
    public MapHouseDataResult queryHouseData(Float lng, Float lat, Integer zoom) {
        double distance = BAIDU_ZOOM.get(zoom) * 1.5 / 111.12; // 1.5 rate is adjustable, depending on your requirement
        Query query = Query.query(Criteria.where("loc").near(new Point(lng, lat)).maxDistance(distance));
        List<MongoHouse> mongoHouses = mongoTemplate.find(query, MongoHouse.class);
